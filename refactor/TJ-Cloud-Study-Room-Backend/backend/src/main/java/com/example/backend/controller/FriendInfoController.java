...
//    }
    // 推荐好友代码重构
//     在重构后的代码中，我们使用Java 8的Lambda表达式和流式操作来简化循环和集合处理。
//     首先，我们使用stream()和filter()函数过滤出非好友用户。
//     然后，使用stream()和count()函数计算重叠课程数目。
//     接下来，我们使用sorted()和limit()函数找出重叠课程数目最多的前三名用户。
//     最后，将推荐用户信息封装为FriendListItem对象，并返回结果。
//     这样做使代码更加简洁、易读和高效。
    @GetMapping("/recommand/friend")
    public Result<?> recommand(@RequestParam Integer uid) {
        List<TimeTable> userTimeTable = timeTableMapper.getUserTimeTable(uid);
        ...
        // 使用Lambda表达式过滤出非好友用户
        List<User> nonFriendUsers = userList.stream()
                .filter(u -> friendInfoMapper.getFriend(uid, u.getUid()) == null)
                .collect(Collectors.toCollection(ArrayList::new));

        // 使用Lambda表达式计算每个非好友用户与被推荐用户的重叠课程数目
        Map<User, Integer> sameCourseNumMap = new HashMap<>();
        for (User user : nonFriendUsers) {
            List<TimeTable> otherTimeTable = timeTableMapper.getUserTimeTable(user.getUid());
            int sameCourseNum = (int) userTimeTable.stream()
                    .filter(ut -> otherTimeTable.stream()
                            .anyMatch(ot -> ot.getStart() == ut.getStart()
                                    && ot.getEnd() == ut.getEnd()
                                    && ot.getWeekday() == ut.getWeekday()))
                    .count();
            sameCourseNumMap.put(user, sameCourseNum);
        }

        // 将重叠课程数目排序，找出前三名
        List<User> recommendedUsers = sameCourseNumMap.entrySet().stream()
        ...
        for (User user : recommendedUsers) {
            FriendListItem item = new FriendListItem();
            item.id = user.getUid();
            item.avatar = user.getPicture();
            item.name = user.getUserName();
            list.add(item);
        }
