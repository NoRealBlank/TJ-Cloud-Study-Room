<template>
  <div class="recommend-page">
    ...
    <div class="friendRecommend-list">
      <!-- 遍历显示好友推荐项 -->
      <div
          v-if="friendRecommends.length > 0"
          v-for="recommend in friendRecommends"
          :key="recommend.id"
          class="friendRecommend-item"
      >
        <!-- 用户信息显示 -->
        <div class="user-info">
        ...
        <el-button
            type="primary"
            @click="sendFriendRequest(recommend)"
            class="add-friend-request"
            :disabled="recommend.requested"
        >
...
export default defineComponent({
    ...
    setup() {
        const friendRecommends = ref([]);//好友推荐的列表
        const failedApplicationErrorMessage = ref('');//申请失败的错误消息提示
        const successfulApplicationCorrectMessage = ref('');//申请成功的正确消息提示
