...
export default {
    ...
    setup(_, { emit }) {
        ...
        const addFriendWindowVisible = ref(false);//添加好友窗口的可视化
        const deleteFriendWindowVisible = ref(false);//删除好友窗口的可视化
        const selectedDeleteFriendIds = ref([]);//选择删除好友的ID列表
        const offlineErrorMessage = ref('');//好友离线的错误消息
        const failedApplicationErrorMessage = ref('');//申请失败的错误消息
        const failedDeletionErrorMessage= ref('');//删除失败的错误消息
        const successfulApplicationCorrectMessage=ref('');//申请成功的正确消息
        const successfulDeletionCorrectMessage= ref('');//删除成功的正确消息
        const addFriendSearchList =ref([]);//添加好友的搜索列表
